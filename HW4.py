...
def task3_kmeans(X: pd.DataFrame, y: pd.Series, classes: list, out_dir: str, fig_dir: str):
    ...
    try:
        from umap import UMAP  # imported lazily to avoid heavy initialization during module import
    except ImportError as exc:  # pragma: no cover - informative failure path
        raise ImportError(
            "UMAP (umap-learn) is required for Task 3 visualizations. "
